          ...
          if(row.owner != 0){
            ...
            db.get(sqlCheckOwner, [], (err, rowO) => {
              ...
              db.run(sqlAddOwner, dataAddOwner, (err) => {
                if(err) return console.error(err.message);
                client.channels.get(config.logging).send(`:speech_balloon: TALKING OWNER : ${message.author.username}#${message.author.discriminator} - ${rowO.money} -> ${rowO.money + Math.floor(random / 4)}`);
